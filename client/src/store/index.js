...
export const GlobalStoreActionType = {
    ...
    SET_LIST_NAME_EDIT_ERROR: "SET_LIST_NAME_EDIT_ERROR",
    EDIT_SONG: "EDIT_SONG",
...
function GlobalStoreContextProvider(props) {
    ...
    const storeReducer = (action) => {
        ...
        switch (type) {
            ...
            case GlobalStoreActionType.SET_LIST_NAME_EDIT_ERROR: {
                return setStore({
                    currentModal: CurrentModal.NONE,
                    idNamePairs: store.idNamePairs,
                    publishedPlaylists: store.publishedPlaylists,
        ...
        async function asyncGetList() {
            ...
            if (response.status === 200) {
                ...
                while (i < playlists.length) {
                    if (playlists[i].name == newListName) {
                        num++;
                        i = 0;
        ...
        async function asyncLoadPublishedPlaylists() {
            ...
            if (response.data.success) {
                ...
                storeReducer({
                    ...
                    payload: publishedPlaylists
                });
                // console.log(publishedPlaylists);
            }
        }
    ...
    store.editNameInUse = function (playlist, newName) {
        console.log(playlist);
        console.log(newName);
        async function asyncGetList() {
            let response = await api.getPlaylistPairs();
            if (response.status === 200) {
                ...
                let i = 0;
                while (i < playlists.length) {
                    if (playlists[i].name == newName && playlist._id != playlists[i]._id) {
                        storeReducer({
                            type: GlobalStoreActionType.SET_LIST_NAME_EDIT_ERROR,
                            payload: null
                        });
